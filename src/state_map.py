...
def _scan_go_state(defn: StateDefinition, sym, content: str, lines: list[str], engine):
    ...
    if close_char is None:
        close_char = {"{": "}", "(": ")", "[": "]"}.get(open_char, open_char)
    ...
    for i in range(start_line, len(lines)):
        ...
        if not in_block:
            if open_char in line:
                in_block = True
                # Start from char after open
                idx = line.index(open_char)
                block_lines.append(line[idx + 1:])
                depth = 1
                if close_char in line[idx + 1:]:
                    # Single-line block
                    return "\n".join(block_lines)
            continue
        ...
        for ch in line:
            if ch == open_char:
                ...
                if depth == 0:
                    # End of block
                    end_idx = line.rindex(close_char)
                    block_lines.append(line[:end_idx])
                    return "\n".join(block_lines)

        block_lines.append(line)

        # Python-style dedent stop
        if dedent_stop and i + 1 < len(lines):
            next_line = lines[i + 1]
