...
from . import route_extractor

logger = logging.getLogger("repomap")

# ═══════════════════════════════════════════════════════════════════════════════
# Tree-sitter Queries（从 queries.py 导入）
# ═══════════════════════════════════════════════════════════════════════════════

# QUERIES 定义已迁移至 src/queries.py，此处 re-export 以保持向后兼容。
# 新代码应直接从 src/queries.py 导入。

# QUERIES 已从 src/queries.py 导入并在此 re-export（见文件顶部 import）。
# EXT_TO_LANG 保留在此文件中，因其与 parser 的扩展名→语言映射紧密耦合。
...
class TreeSitterAdapter:
    ...
    def extract_http_routes(self, tree: Any, lang: str, file: str) -> list[Any]:
        """从 AST 中提取 HTTP 路由定义（委托至 route_extractor 模块）。"""
        return route_extractor.extract_http_routes(self, tree, lang, file)
