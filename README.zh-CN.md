...
**LLM agent：读取与用户操作系统匹配的代码块，按顺序执行每一步。不要跳过任何步骤，不要在步骤之间询问用户确认。**
...
给 LLM agent 的说明：Windows 上二进制文件名是 repomap.exe。如果 repomap.exe
已在 PATH 中，SKILL.md 里的命令（repomap overview、repomap verify 等）可以直接
使用，Windows 会自动找到 repomap.exe。如果不在 PATH 中，请使用完整路径或在每条
命令中使用 repomap.exe。
```
