...
class TestGitBackend:
    def test_selects_pygit2_when_available_and_repo_ok(self) -> None:
        with patch("src.git_backend._HAS_PYGIT2", True), \
    ...
    def test_delegates_rev_parse_head(self) -> None:
        with patch("src.git_backend._HAS_PYGIT2", False), \
             patch.object(SubprocessBackend, "rev_parse_head", return_value="abc12345") as m:
            gb = GitBackend("/fake")
    ...
    def test_delegates_status_porcelain(self) -> None:
        with patch("src.git_backend._HAS_PYGIT2", False), \
             patch.object(SubprocessBackend, "status_porcelain", return_value=["M file.py"]) as m:
            gb = GitBackend("/fake")
...
class TestSubprocessBackend:
    ...
    def test_rev_parse_head_success(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("a1b2c3d4e5f6\n")):
            assert SubprocessBackend.rev_parse_head(self.ROOT) == "a1b2c3d4e5f6"

    def test_rev_parse_head_failure(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed(returncode=128)):
    ...
    def test_show_toplevel_success(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("/home/user/repo\n")):
    ...
    def test_changed_files_diff_and_untracked(self) -> None:
        ...
        with patch.object(SubprocessBackend, "_run_git", side_effect=[diff_result, ls_result]):
    ...
    def test_deleted_files(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("old.py\ngone.py\n")):
            assert SubprocessBackend.deleted_files(self.ROOT) == ["old.py", "gone.py"]

    def test_deleted_files_none(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("")):
            assert SubprocessBackend.deleted_files(self.ROOT) == []

    # diff_name_only

    def test_diff_name_only_without_since(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("a.py\nb.py\n")):
            assert SubprocessBackend.diff_name_only(self.ROOT) == ["a.py", "b.py"]

    def test_diff_name_only_with_since(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("c.py\n")) as m:
            result = SubprocessBackend.diff_name_only(self.ROOT, since="v1.0")
            assert result == ["c.py"]
            m.assert_called_once_with(["diff", "--name-only", "v1.0", "HEAD"], self.ROOT)
    ...
    def test_diff_cached_name_only(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("staged.py\n")):
    ...
    def test_status_porcelain_various_codes(self) -> None:
        ...
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed(output)):
    ...
    def test_log_name_only_custom_since(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("c.py\n")) as m:
            ...
            assert result == ["c.py"]
            m.assert_called_once_with(
                ["log", "--name-only", "--pretty=format:", "--since=30.days.ago", "--", "."],
                self.ROOT,
            )
    ...
    def test_blame_line_success(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("a1b2c3d4 some rest\n")):
...
class TestPygit2Backend:
    ...
    def test_repo_failure_returns_none(self) -> None:
        with patch("src.git_backend._HAS_PYGIT2", True), \
             patch("src.git_backend.pygit2.Repository", side_effect=Exception("no repo")):
    ...
    def test_repo_fallback_discover_repository(self) -> None:
        mock_repo = self._make_mock_repo()
        with patch("src.git_backend._HAS_PYGIT2", True), \
             patch("src.git_backend.pygit2.Repository", side_effect=[Exception("not found"), mock_repo]), \
             patch.object(Pygit2Backend, "show_toplevel", return_value="/discovered/repo"):
    ...
    def test_rev_parse_head_exception(self) -> None:
        mock_repo = self._make_mock_repo()
        type(mock_repo.head).target = property(lambda self_: (_ for _ in ()).throw(Exception("no head")))
        with patch.object(Pygit2Backend, "_repo", return_value=mock_repo):
            assert Pygit2Backend.rev_parse_head(self.ROOT) is None
...
class TestContractConsistency:
    ...
    def test_blame_line_subprocess_returns_8char_hash(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("a1b2c3d4e5f6 rest\n")):
    ...
    def test_blame_line_both_backends_same_key(self) -> None:
        with patch.object(SubprocessBackend, "_run_git", return_value=_mock_completed("a1b2c3d4 rest\n")):
