...
class RepoMapToolkitTests(unittest.TestCase):
    ...
    def test_refs_and_orphans_count_import_edges_as_references(self) -> None:
        with tempfile.TemporaryDirectory() as project_root:
            ...
            # import 边计入引用计数，不再误判为 orphan
            self.assertEqual(refs["ref_count"], 1)
            self.assertFalse(refs["is_entry"])
            self.assertTrue(refs["is_leaf"])
            self.assertNotIn("shared_helper", orphan_names)
